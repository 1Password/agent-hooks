#!/usr/bin/env bash
#
# Install agent hooks for Cursor, GitHub Copilot, or Claude Code.
...
# Run from this repo.
#
# Usage: ./install.sh --agent cursor|github-copilot|claude-code [--target-dir DIR]
#
set -euo pipefail
...
usage() {
  echo "Usage: $0 --agent cursor|github-copilot|claude-code [--target-dir DIR]"
  echo ""
  echo "  --agent      (required) Agent (cursor, github-copilot, or claude-code)."
...
require_value() {
  local opt="$1"
  if [[ $# -lt 2 || -z "$2" || "$2" == -* ]]; then
    echo "Error: $opt requires a value (e.g. for --agent: cursor, github-copilot, claude-code)" >&2
    exit 1
  fi
}
...
if [[ -z "$AGENT" ]]; then
  echo "Error: --agent is required (cursor, github-copilot, or claude-code)" >&2
  usage
fi
