...
Every time Cursor attempts to execute a shell command such as `echo`, `grep`, `ls`, etc., the [`1password-verify-environments.sh`](./1password-verify-environments.sh) script will run and query 1Password for its configured [local .env files](https://developer.1password.com/docs/environments/local-env-file). It will then validate that each path is enabled, and exists as a valid FIFO (named pipe). When validation fails, the hook blocks command execution and provides clear error messages indicating which files are missing or need to be enabled from the 1Password app. The Cursor Agent will then guide you towards a proper configuration.
